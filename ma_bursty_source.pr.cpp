...
const char ma_bursty_source_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 57EF10B9 57EF10B9 1 Loren Loren 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                              ";
#include <string.h>
...
bursty_source_sv_init ()
{
	...
	int ID = atoi(temp) - 1;
	printf("ID = %d restart = %d\n", ID, vidData[ID].restart);
					...
					if (op_sim_time () >= EAestimationTimeApp)//if EA estimation time is done
					{
						...
						if(frameSize != 0)
						{
							...
							do
							{
								...
								printf("trying to pFrame data\n");
								//av_freep(&pFrame->data[0]);
								
								av_frame_free(&pFrame);
								
								printf("trying to frame data\n");
