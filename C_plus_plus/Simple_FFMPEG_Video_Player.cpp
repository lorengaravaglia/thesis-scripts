...
{
                                         //"G:\\Masters_Thesis_Files\\Honda_Database\\Database1\\Training\\videos\\behzad\\behzad.avi";

	av_register_all();
	avformat_network_init();
	pFormatCtx = avformat_alloc_context();
	//av_dict_set(&stream_opts, "timeout", "100000000", 0); // in microseconds.
	if(avformat_open_input(&pFormatCtx,filepath,NULL,NULL)!=0)
	{
		printf("Couldn't open input stream.\n");
		return -1;
	...
	//av_dict_set(&stream_opts, "timeout", "100000000", 0);

	for(;;)
	{
	printf("reading frame.\n");
	returnValue = av_read_frame(pFormatCtx, packet);
